...
import { ParsedPipelineResolverInfo, ParsedUnitResolverInfo } from '../lib/parser/types.js';
import { loadCdk } from './utilts.js';
import ValidationError from '../lib/ValidationError.js';
import { invalidCustomStructure, invalidDefaultRoot, validCustomStructure, validDefaultRoot } from './constants.js';
import { DataSourceNotFoundError } from '../lib/loader/errors.js';
...
describe('Test VTL loading using default and custom structures', function () {
    invalidCases.forEach(({ options, type }) => {
        it(`Should throw an error when loading using an invalid structure (${type})`, function () {
            expect(() => loadCdk(options))
                .to.throw(ValidationError);
        });
    });

    it('Should throw an error when an invalid AppSync function name is specified in pipeline seq', function () {
        expect(() => loadCdk({ structure: { root: 'test/cases/invalid/invalidFunction'}}))
            .to.throw(/Unable to find AppSync function 'WhatFunctionItDoesNotExist'/i)
    });

    it('Should throw an error when an invalid data source is specified in an AppSync function', function () {
        expect(() => loadCdk({ structure: { root: 'test/cases/invalid/invalidFunctionDs'}}))
            .to.throw(/Data source 'nods' was not found/i)
    });

    it('Should not create more than one NONE data source when multiple none data source resolvers exist', function () {
        const { dummyStack, api } = loadCdk({ structure: { root: 'test/cases/valid/manyNoneDs' } });
        const template = Template.fromStack(dummyStack);

        template.resourceCountIs('AWS::AppSync::Resolver', 2);
        template.resourceCountIs('AWS::AppSync::DataSource', 3); // 2 existing data sources and 1 none ds
    ...
    validCases.forEach(({ options, type }) => {
        it(`Should build resolvers (${type})`, function () {
            const { loader } = loadCdk(options);
        ...
        it(`Should load resolvers (${type})`, function () {
            const { dummyStack, api: graphqlApi, loader } = loadCdk(options);
            const template = Template.fromStack(dummyStack);
        ...
        it(`Should load functions (${type})`, function () {
            const { dummyStack, api: graphqlApi, loader, dynamoDbTable, secondaryDynamoDbTable } = loadCdk(options);
            const template = Template.fromStack(dummyStack);
        ...
        it(`Should throw an error when a default data source is required (${type})`, function () {
            expect(() => loadCdk(options, true))
                .to.throw(DataSourceNotFoundError, /No default (function|unit resolver) data source was provided/i)
        });

        it(`Should throw an error when a default function data source is required (${type})`, function () {
            expect(() => {
                loadCdk({
                    ...options,
        ...
        it(`Should throw an error when a default unit resolver data source is required(${type})`, function () {
            expect(() => {
                loadCdk({
                    ...options,
        ...
        it(`Should use 'none' as a default function data source (${type})`, function () {
            const beans = loadCdk({
                ...options,
                structure: { ...options.structure, root: `test/cases/valid/defaultFunctionDataSource/${type}` },
        ...
        it(`Should use 'none' as a default unit resolver data source (${type})`, function () {
            const beans = loadCdk({
                ...options,
                structure: { ...options.structure, root: `test/cases/valid/defaultUnitDataSource/${type}` },
